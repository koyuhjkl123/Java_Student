...
//		유저에게 선택권을 준다.
	...
	@Override
	public void UserNonMemberLogin(String key) {
//		비 회원 로그인하기
		System.out.println("받으신 인증 번호 입력 : ");
		String in_key = sc.next();

		if (key.equals(in_key)) {
			System.out.println("User님 로그인 하셨습니다.");

			System.out.println("비 회원 로그인은 게임은 가능 하시나 실버 등급까지 게임이 가능합니다.");
			System.out.println("그 이후 등급은 게임이 불가합니다.");
		} else {
...
//		비회원 로그인은 랜덤 키로 받은 값으로만 로그인이 가능하다.
		sql = "insert into UserNonMember(num, approval_key) values(null, ?) ";
...
//		회원가입
		...
		while (true) {
			...
			if (UserRegisterIs(getUserid())) {
				...
				setUsergrade(UserGrade.Bronze.getGrade()); // 초기 회원 등급은 Bronze이다.
				break;
			}
		}
		...
		try {
			...
			pstmt.setString(8, getUsergrade());
...
//		러시안 룰렛 게임!
	...
	@Override
	public void UserBollets(String user_id) {
		String sql = "select name, Grade from UserMerber where id = ?";
//		러시안 룰렛 게임!
...
//			자신에게 쏘는 경우
					if (bulletType.equals("실탄")) { // 내 자신에게 쏘는데 실탄이라면 자기 체력 1감소
						...
						health.put(user_info.getString(1), health.get(user_info.getString("name")) - 1);
						System.out.println("내 체력 : " + health.get(user_info.getString("name")));
						AdminRussianRoulett(user_id); // 봇에게 턴
						break;
